...
| 5 | BBS **핵심 5기능**(목록·상세·작성·수정·삭제)을 **직접 코드로** 완성한다 | AI 도움 없이 라우트·SQL을 손으로 작성해 동작시킴 |
...
**핵심 기능(목록·상세·작성·수정·삭제)은 AI 없이 직접 코드를 작성**해서 완성한다.
AI는 오늘 마지막 도전 과제에서, 스스로 상상한 기능·스타일을 구현할 때만 쓴다.
(회원가입·로그인·권한으로 업그레이드하는 것은 **7주차**.)
...
| 4 | **수정** (`GET /posts/<id>/edit`, `POST /posts/<id>/edit`) | 기존 글 내용을 고쳐서 저장 |
| 5 | **삭제** (`POST /posts/<id>/delete`) | 글이 DB에서 제거되고 목록에서 사라짐 |
| 6 | **DB** | `posts` 테이블에 글이 실제로 쌓임 (서버 재시작 후에도 유지) |
...
3. 글을 수정하면 내용이 바뀌고, 삭제하면 목록에서 사라진다
4. 서버를 껐다 켜도 글이 **그대로 남아 있다** (DB에 저장됐다는 증거)
5. 클라이언트-서버-DB 그림을 보고 오늘 만든 BBS의 각 부분을 손으로 가리킬 수 있다
...
| 1:25~1:40 | — | — | 휴식 |
| 1:40~2:50 | C | 5 | 🟢 목록·상세·작성·수정·삭제 **직접 코드로** 구현 (AI 없이) |
| 2:50~3:00 | — | — | 휴식 |
| 3:00~3:45 | D | 6 · 7 | 🟡 손으로 한 군데 고치기·스타일 · 🔴 AI 도전 과제 |
| 3:45~4:00 | E | 8 | 3단 구조 발표 · 회고 · Week7 예고 |
...
### 2.2. 오늘의 최소 BBS 3화면 + 2기능

```
[ 목록 ]  GET /                    → 저장된 글 제목 리스트
[ 상세 ]  GET /posts/<id>          → 글 하나의 제목+본문
[ 작성 ]  GET /new (폼) + POST /posts (저장)
[ 수정 ]  GET /posts/<id>/edit (폼) + POST /posts/<id>/edit (저장)
[ 삭제 ]  POST /posts/<id>/delete
```

수정·삭제는 새로운 화면 유형이 아니라, **상세·작성과 같은 패턴**(GET으로 보여주고 POST로 DB를 바꾼다)의 반복이다.
...
## 5. BBS 핵심 기능 직접 만들기 (🟢, AI 없이)

> 오늘은 AI에게 "만들어줘"라고 시키지 않는다. 아래 코드를 **한 줄씩 이해하며 직접 입력**한다.
> 막히면 챕터 3~4(라우트·SQL 개념)로 돌아가 확인한다 — AI는 오늘 마지막 도전 과제에서만 쓴다.
...
목록·상세·작성과 **똑같은 패턴**이다 — "화면 보여주기(GET)"와 "DB 바꾸기(POST)"만 반복된다.
...
- `UPDATE ... WHERE id = ?`에서 `WHERE`를 빼면 어떻게 될까? → **모든 글**이 같은 내용으로 바뀐다 (직접 지워봤다가 다시 넣어서 확인해도 좋음)
...
`templates/edit.html` (`new.html`과 거의 같고, `value`로 기존 값을 미리 채워 넣는다)
```html
<!DOCTYPE html>
<html lang="ko">
...
> 이 코드를 직접 입력해 본다. 안 되는 부분이 있으면 오타·라우트 이름·`?` 개수부터 스스로 확인한다 — 지금은 AI에게 대신 고쳐 달라고 하지 않는다.
...
- [ ] 상세에서 "삭제"를 누르면 글이 목록에서 사라진다
- [ ] 서버를 껐다 켜도 남은 글이 유지된다 (DB 저장 확인)

---

## 6. 🟡 손으로 한 군데 고치기 + 최소 스타일 (직접, AI 없이)

### 6.1. 손으로 한 군데 고치기 (필수 습관)
...
아래 CSS를 각 템플릿의 `<head>` 안에 `<style>...</style>`로 직접 붙여 넣는다 (AI에게 시키지 않고 값만 바꿔가며 감을 익힌다).
...
## 7. 🔴 도전 과제 — AI와 함께 상상한 기능·스타일 만들기

지금까지는 전부 직접 코드를 작성했다. 이제부터는 **AI를 처음 사용**한다 — 이미 라우트·SQL·템플릿이 어떻게 동작하는지 알기 때문에, AI가 만든 코드도 읽고 판단할 수 있다.
...
### 7.3. 예시 아이디어 — fetch로 비동기 글쓰기 (React로 가는 다리)

지금까지는 폼을 제출하면 **페이지 전체가 새로고침**됐다. `fetch`를 쓰면 페이지를 새로고침하지 않고도 서버와 데이터를 주고받을 수 있다 — 7주차 이후 기능 확장(댓글 등)에서도 같은 방식을 쓸 수 있다.
...
### 7.4. 자유 상상 — 나만의 기능/스타일 1개

내가 만들고 싶은 것을 한 줄로 먼저 적어본다 (예: "댓글을 달고 싶다", "다크 테마로 바꾸고 싶다", "글마다 태그를 달고 싶다"). 그다음 7.1의 틀에 채워 넣어 AI에게 요청한다. **결과 코드를 받으면 반드시 한 줄씩 읽고, 모르는 부분은 AI에게 설명해 달라고 물어본다.**
...
- [ ] 목록·상세·작성·수정·삭제가 서로 연결됨 (AI 없이 직접 작성)
- [ ] `?` 파라미터로 안전하게 값 전달하는 습관

### 8.5. 다음 주 예고 (Week7)

- 오늘 만든 BBS에 **회원가입·로그인·로그아웃** 추가
- 글에 **작성자** 연결, **로그인해야 글쓰기**, **본인 글만** 수정·삭제
- 4주차 Permission — 「이 행동을 해도 될까?」가 실제 코드가 됨

미리 생각해 오기 (숙제 아님):
> 지금은 아무나 아무 글이나 고칠 수 있다. 실제 학교 게시판이라면 어떤 규칙이 필요할까?

---

## 🟢🟡🔴 과제 카드

### 🟢 (필수 · 수업 중 완료 목표 · AI 없이 직접 작성)
- [ ] Flask 서버 로컬 실행
- [ ] 목록·상세·작성·수정·삭제 5기능 동작
- [ ] SQLite에 글이 저장·수정·삭제되고 유지됨
...
### 진행 팁
- **오늘 챕터 5(핵심 기능)는 AI를 의도적으로 배제한다.** 학생이 "AI한테 시키면 되잖아요"라고 하면 — 7장(도전 과제)에서 AI를 쓸 텐데, 그때 AI가 만든 코드를 읽고 판단하려면 오늘 라우트·SQL이 뭘 하는지 손으로 먼저 알아야 한다고 설명한다.
- 사전과정(1~2주차)에서 Flask 챗 서버를 이미 만들어본 학급이면 **챕터 1~4를 빠르게** 훑고 5(핵심 기능)에 시간을 더 준다. 처음인 학급이면 챕터 3~4에서 속도를 늦춘다.
...
- 챕터 5에서 시간이 많이 남는 학생은 6장(스타일)으로 먼저 보내고, 7장(AI 도전)은 5·6이 끝난 학생만 진행하게 한다.
- **시간 내에 5기능(목록·상세·작성·수정·삭제)을 못 끝낸 학생:** 억지로 붙잡지 않는다. 수업 종료 직전에 [starters/reference/week06-complete/](../starters/reference/week06-complete/)의 `app.py`·`templates/`를 **따라잡기용으로 전달**하고, "시간 남을 때 자기 코드와 한 줄씩 비교해 보라"고 안내한다. 7주차는 이 상태를 그대로 이어받으므로, 완성 여부와 무관하게 전원이 같은 출발선에서 다음 주를 시작할 수 있게 하는 것이 목적이다.
...
오늘 결과 → 목록·상세·작성·수정·삭제 BBS, 전부 직접 작성 (로컬)
AI는 마지막 도전 과제(7장)에서만 사용
다음 → 회원가입·로그인·권한으로 BBS v1 업그레이드 (Week7)
```
...
A. 오늘 핵심은 **저장·조회·수정·삭제가 되는 구조**다. 스타일은 🟡 — 5기능이 안 되면 먼저 그것부터.

**Q. 왜 오늘은 AI를 안 쓰나요?**
A. AI가 만들어 준 코드를 "맞는지 판단"하려면 그 코드가 뭘 하는지 먼저 알아야 한다. 오늘 직접 쓴 라우트·SQL이 다음 주(디자인 패턴)와 7장(AI 도전 과제) 모두의 기반이 된다.

**Q. React를 오늘 바로 써도 되나요?**
A. 캡스톤·보충 자료에서 다룰 수 있다. 6~7주차는 Flask 템플릿(Jinja)만으로 충분 — **기능(저장·회원·권한)**을 먼저 익힌다.

---

## 부록 A — 학생용 프롬프트 치트시트 (7장 도전 과제 전용 — 그 전에는 AI 사용 안 함)

```
[기본 틀]
목표: 오늘 만든 BBS에 (기능/스타일)을 추가한다.
제약: 기존 목록/상세/작성/수정/삭제는 유지. app.py, templates/ 안에서만 수정.
먼저 계획만 제안해줘. 승인하면 그때 구현해줘.
...
[자유 상상 🔴]
(내가 상상한 기능/스타일 한 줄)을 위 기본 틀에 채워 요청.
```
...
| 수정해도 내용이 안 바뀜 | `UPDATE` 문의 `SET` 컬럼과 `?` 순서 불일치 | `title = ?, content = ?` 순서와 값 순서 다시 확인 |
| 삭제했는데 다른 글이 없어짐 | `DELETE`에 `WHERE id = ?` 누락 | 반드시 `WHERE`로 특정 글만 지정 |

---

## 다음 주 (Week07)로 이어지는 다리

오늘은 **동작하는 BBS(v0)**까지다 — 그런데 누가 썼는지 모르고, 아무나 글을 고칠 수 있다.
다음 주는 **회원가입·로그인·세션·권한**을 추가해, "내가 쓴 글만 고칠 수 있는" **BBS v1**으로 업그레이드한다.
